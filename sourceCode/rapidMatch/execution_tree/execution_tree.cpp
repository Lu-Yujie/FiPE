...
bool execution_tree::execute(catalog &catalog, uint64_t output_count_limit, size_t& call_cnt,
                                 size_t& embedding_cnt, int64_t& time_limit) {
    bool overtime = false;
    for (auto node : tree_nodes_) {
        overtime = node->execute(catalog, relations_, output_count_limit, call_cnt, embedding_cnt, time_limit);
        if (overtime == true) {
            return overtime;
        }
    }
