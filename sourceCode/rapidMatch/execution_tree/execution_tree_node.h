...
protected:
    ...
    virtual bool execute_operator(std::vector<flat_relation *> &relations, size_t& call_cnt,
                                  size_t& embedding_cnt, int64_t& time_limit) = 0;
...
public:
    execution_tree_node(int id, int parent, join_operator_type type, std::vector<uint32_t>& vertices)
            : id_(id), parent_(parent), operator_type_(type), vertices_(vertices), execution_time_(0) {}

    execution_tree_node() : execution_time_(0) {}
    ...
    bool execute(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit,
                     size_t& call_cnt, size_t& embedding_cnt, int64_t& time_limit) {
        bool overtime = false;
        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        overtime = execute_operator(relations, call_cnt, embedding_cnt, time_limit);
...
protected:
    ...
    bool execute_operator(std::vector<flat_relation *> &relations, size_t& call_cnt,
                          size_t& embedding_cnt, int64_t& time_limit) override;
