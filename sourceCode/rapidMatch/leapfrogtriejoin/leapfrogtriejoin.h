...
private:
    ...
    bool enumerate_non_core_results(uint32_t start_depth, uint32_t max_depth, size_t& call_cnt,
...
public:
    leapfrogtriejoin(flat_relation* input, flat_relation* output, uint32_t* vertex_ordering,
                     ...
                     uint64_t output_count_limit = std::numeric_limits<uint64_t>::max()) :
                             ...
                             output_count_limit_(output_count_limit), materialize_(false) {
        initialize();
        initialize_bn_fn();
    ...
    bool execute(size_t& call_cnt, size_t& embedding_cnt, int64_t& time_limit);
