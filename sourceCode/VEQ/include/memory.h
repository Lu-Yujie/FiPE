...
int nCandidate = 0;
mpq_t nMatch, nCurrMatch, nRemainingMatch;
mpq_t mpqOne, mpqTmp, mpqZero;
#ifdef SUBGRAPH_MATCHING
mpq_t nMaxMatch;
#else
...
inline void AllocateForQueryGraph(Graph& query) {
  ...
  for (int i = 0; i < nQueryVertex; i++) {
    ...
    element[i].neverVisited = new bool[maxNumCandidate]();  // TODO. to
                                                            // maxDegree
    element[i].nFoundMatch = new mpq_t[maxNumCandidate];
    for (int lu_i = 0; lu_i < maxNumCandidate; lu_i++) {
      mpq_init(element[i].nFoundMatch[lu_i]);
      mpq_set_d(element[i].nFoundMatch[lu_i], 0);
    }
#endif
#endif
#ifdef LEAF_ADAPTIVE_MATCHING
