...
inline void GetEquivalence(Stack& retE) {
  ...
  for (aPos = initPos; aPos < retE.addressSize; ++aPos) {
...
#ifdef SUBGRAPH_MATCHING
    if ((mpq_cmp(retE.nFoundMatch[initPos], mpqZero) > 0 && isVisited[v] == nCell &&
         retE.neverVisited[aPos]) ||
        (mpq_cmp(retE.nFoundMatch[initPos], mpqZero) == 0 && isVisited[v] == nCell))
#else
...
#endif
    {
      ...
      mpq_set(retE.nFoundMatch[aPos], retE.nFoundMatch[initPos]);
#ifdef MAPPING_FUNCTION_LOG
...
inline void combine(double& cntEmbedding, mpq_t resultSoFar) {
  if (GetClockNan() > endTime) {
    exceedTimeLimit = true;
    return;
  }
#ifdef SUBGRAPH_MATCHING
  mpq_set_d(mpqTmp, cntEmbedding);
  mpq_mul(mpqTmp, mpqTmp, resultSoFar);
  if (mpq_cmp(mpqTmp, nRemainingMatch) > 0)
  ...
  for (int i = 0; i < vCandIndex[maxCand]; i++) {
    ...
    mpq_t recurResult;
    mpq_init(recurResult);
    mpq_set(recurResult, resultSoFar);
    combine(cntEmbedding, recurResult);  // recursive function
    mpq_clear(recurResult);
#ifdef SUBGRAPH_MATCHING
    mpq_set_d(mpqTmp, cntEmbedding);
    mpq_mul(mpqTmp, mpqTmp, resultSoFar);
    if (mpq_cmp(mpqTmp, nRemainingMatch) > 0)
...
inline void CountLeafMatch(const Graph& query, mpq_t matchFound, mpq_t matchResult) {
#ifdef SUBGRAPH_MATCHING
  mpq_sub(nRemainingMatch, nMaxMatch, matchFound);
#endif
  mpq_set_d(matchResult, 0);
  ...
  for (int i = 0; i < NECSetSize; i++) {  // for each nec node label
#ifdef SUBGRAPH_MATCHING
    if (mpq_cmp(matchResult, nRemainingMatch) > 0) continue;
#else
    if (mpq_cmp_ui(matchResult, 0, 0) > 0) continue;
#endif
    ...
    if (nodeSum == 1) {
      ...
      mpq_t recurResult;
      mpq_init(recurResult);
      mpq_set(recurResult, matchResult);
      combine(cntLocalMatchLabel, recurResult);
      mpq_clear(recurResult);
    ...
    mpq_set_d(mpqTmp, cntLocalMatchLabel);
    mpq_mul(matchResult, matchResult, mpqTmp);
  }
#ifdef MAPPING_FUNCTION_LOG
  printf("[L] Found %Qd embeddings.\n", matchResult);
#endif
  return;
}
...
inline void reset(int depth, int ri, int rootCand) {
  while (depth > 0) {
    ...
    mpq_sub(higherE.nFoundMatch[higherE.addressPos],
...
#ifdef SUBGRAPH_MATCHING
  mpq_sub(rootE.nFoundMatch[ri], nMatch, rootE.nFoundMatch[ri]);
  Deactivate(rootE, 0);
#endif
...
#ifdef SUBGRAPH_MATCHING
  if (mpq_cmp(nMatch, nMaxMatch) >= 0) return true;
#else
...
inline void Backtrack(const Graph& query, const Graph& data, int dataGraphID) {
...
#endif
  {
...
#ifdef SUBGRAPH_MATCHING
      mpq_add(nMatch, nMatch, rootE.nFoundMatch[ri]);
#endif
      continue;
    } else {
#ifdef SUBGRAPH_MATCHING
      mpq_set(rootE.nFoundMatch[ri], nMatch);
    ...
    char backtrack = 0;
#ifdef MAPPING_FUNCTION_LOG
    gmp_printf("[0] 3(R). M[u%d]: v%d, Mark v%d as visited (%Qd matches)\n", root,
...
// do not return now continue to the roll back process
        ...
        if (query.numNECMapping != 0) {
          CountLeafMatch(query, nMatch, nCurrMatch);
#ifdef SUBGRAPH_MATCHING
          mpq_add(nMatch, nMatch, nCurrMatch);
#else
          if (nCurrMatch > 0 && !answer[dataGraphID]) {
            mqp_add(nMatch, nMatch, mpqOne);
            answer[dataGraphID] = true;
          }
#endif
        } else {
          mpq_set_d(nCurrMatch, 1);
#ifdef SUBGRAPH_MATCHING
          mpq_add(nMatch, nMatch, mpqOne);
#else
          if (!answer[dataGraphID]) mqp_add(nMatch, nMatch, mpqOne);
#endif
          answer[dataGraphID] = true;
        }
#ifdef MAPPING_FUNCTION_LOG
        gmp_printf(
            "For v%d(pos: %d)(g%d) in C(r), N(total match): %Qd, N(match just "
            "found): %Qd\n",
...
// no recursive call count increase here!
        ...
        if (mapTo[vID] < 0)  // if v is unvisited
        {
          ...
          mpq_set(currE->nFoundMatch[currE->addressPos], nMatch);
          if (cellID > 0) {
